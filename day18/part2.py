...
with open('day18/input.txt', 'r') as f:
    number_sources = [line.strip() for line in f]

# Brute forcing this; it won't be too bad (100x99)
max_magnitude = 0
for i in range(len(number_sources)):
    for j in range(len(number_sources)):
        if i != j:
            # Note: creating from string source because my processing is destructive :(
            number = SnailNumber(DataReader(number_sources[i])).plus(SnailNumber(DataReader(number_sources[j])))
            resolve(number)
